...
function eliminarRegistro($id) {
    ...
    if ($stmt->execute()) {
        ...
        echo "<meta http-equiv='refresh' content='3;URL=chofer.php'>";
    }
}

// Manejo de la inserción de registros
if (isset($_POST['btnAgg'])) {
    agregarRegistro($_POST['nombre'], $_POST['ficha'], $_POST['no_licencia']);
}

// Función para agregar un registro
function agregarRegistro($nombre, $ficha, $no_licencia) {
    global $conn;
    $sql = "INSERT INTO choferes (nombre, ficha, no_licencia) VALUES (?, ?, ?)";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("sss", $nombre, $ficha, $no_licencia);
    if ($stmt->execute()) {
...
if (isset($_POST['btnEditar'])) {
    editarRegistro($_POST['id'], $_POST['nombre'], $_POST['ficha'], $_POST['no_licencia']);
}

// Función para editar un registro
function editarRegistro($id, $nombre, $ficha, $no_licencia) {
    global $conn;
    $sql = "UPDATE choferes SET nombre=?, ficha=?, no_licencia=? WHERE id=?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("sssi", $nombre, $ficha, $no_licencia, $id);
    if ($stmt->execute()) {
        echo '
        <center>
        <div class="alert">
            ...
            <table>
                ...
                <td><?php echo ++$count; ?></td>
                  <form action="#"  method="post">
                    ...
                    type="text" name="no_licencia" value="<?php echo $row['no_licencia']?>"></td>
...
<script type="text/javascript">
    function preguntar(id) {
        if (confirm('¿Estás seguro que deseas eliminar esto?')) {
            window.location.href = "chofer.php?id=" + id;
        }
    }
